...
def playback_thread_func(playback_stream):
    while True:
...
def start_recording(model='medium', non_english='false', energy_threshold=690, record_timeout=1, phrase_timeout=1, default_microphone='pulse', output_filename='output_audio.wav'):
    phrase_time = None
    data_queue = Queue()
    ...
    playback_thread = Thread(target=playback_thread_func, args=(playback_stream,))
    playback_thread.start()
